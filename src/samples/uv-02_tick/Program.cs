...
internal static unsafe class Program
{
    ...
    {
        Console.WriteLine(
            "Tick example: demo of event loop callback flow, useful as an example of simplistic base application with `libuv`.");
        Console.WriteLine(
            "Press `x` to gracefully exit. Press `CTRL-C` to hard exit.");
    ...
    private static void TryCloseHandle(uv_handle_t* handle, void* param)
    {
        ...
        string handleTypeName = uv_handle_type_name(handleType);
